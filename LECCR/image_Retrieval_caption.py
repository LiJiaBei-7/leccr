...
def train(model, data_loader, optimizer, tokenizer, epoch, device, scheduler, config):
    ...
    for i, (image, text_ls, captions, idx, cap_idx) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_itc_vs, loss_itc_vt, loss_itc_st, loss_itc_c, loss_reg_c = model(image, text_inputs, captions=captions, idx=idx, epoch=epoch, cap_idx=cap_idx)
...
def evaluation_coarse(model, data_loader, tokenizer, device, config):
    ...
    for image, generated_captions, img_id in data_loader:
        ...
        image_feat,_, _ = model.interaction_with_caption(image_embeds=image_feat, caption_embeds=caption_embed, key_padding_mask=key_padding_mask)
...
def evaluation_coarse_caption(model, data_loader, tokenizer, device, config):
    model.eval()
    ...
    for image, generated_captions, img_id in data_loader:
        ...
        image_feat,caption_embeds, ori_caption_embeds = model.interaction_with_caption(image_embeds=image_feat, caption_embeds=caption_embed, key_padding_mask=key_padding_mask)
        image_feat = image_feat.transpose(0,1).contiguous()
        ########  generated caption  ########
    ...
    alpah = 0.8
    score_matrix_i2t = alpah * score_matrix_i2t + (1-alpah) * c_score_matrix_i2t
    score_matrix_t2i = alpah * score_matrix_t2i + (1-alpah) * c_score_matrix_t2i
...
def main(args, config):
    ...
    train_dataset, val_dataset_dict, test_dataset_dict = create_dataset('re_caption', config)
    ...
    #     score_test_i2t, score_test_t2i = evaluation_coarse_caption(model_without_ddp, test_loader, tokenizer, device, config)
