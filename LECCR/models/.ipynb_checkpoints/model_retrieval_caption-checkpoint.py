...
class RetrievalModel(XVLMBase):
    ...
    def init_caption_encoder(self):
        with torch.no_grad():
            import copy
            self.caption_encoder = copy.deepcopy(self.text_encoder)
        self.caption_proj = nn.Linear(self.text_width, self.vision_width)
        
        from models.attention import CrossAttention
        self.crossattn = CrossAttention(d_model=self.vision_width, nhead=self.num_attention_heads, num_layers=1)
...
#         return aux_contrastive_loss
    ...
    def forward(self, image, text_ls, captions=None, idx=None, epoch=None, cap_idx=None):
        ...
        image_embeds = self.crossattn(image_embeds.transpose(0,1).contiguous(), caption_embeds.transpose(0,1).contiguous())
        image_embeds = image_embeds.transpose(0,1).contiguous()
        ########  generated caption  ########
