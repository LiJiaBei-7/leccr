import torch
from models import XVLMBase_video as XVLMBase, load_pretrained

import torch.nn.functional as F
import torch.nn as nn

from models.attention import CrossAttention, SelfAttention
# 引入caption_reg_loss + cv_loss + share_encoder + dstl
...
class RetrievalModel(XVLMBase):
    def __init__(self, config):
        ...
        self.vid_encoder = SelfAttention(d_model=self.vision_width, nhead=8, num_layers=config['vision_layer'])
    

    def get_vision_embeds(self, video, mask):
        mask = ~mask
        vid_feat = self.vid_encoder(video.transpose(0,1), memory_key_padding_mask=mask).transpose(0,1)

        return vid_feat, mask
    ...
    def caption_regularization_1(self, caption_embeds):
        # caption_embeds n,bsz,d
        bsz = caption_embeds.size(1)
        caption_embeds = F.normalize(caption_embeds, p=2, dim=-1)
        caption_embeds = caption_embeds.transpose(0,1)
        sim_matrix = caption_embeds @ caption_embeds.transpose(1,-1) # bsz, n, n
        sim_matrix = torch.softmax(sim_matrix, dim=-1)
        diagonals = torch.log(torch.diagonal(sim_matrix, dim1=-2, dim2=-1))
        
        return -diagonals.mean()

    
    def interaction_with_caption(self, image_embeds, caption_embeds, key_padding_mask, video_mask):
        ...
        after_caption_embeds = self.crossattn2(caption_embeds, image_embeds.transpose(0,1).contiguous(), memory_key_padding_mask=video_mask)
    ...
    def forward(self, video, video_mask, text_ls, captions=None, idx=None, epoch=None, cap_idx=None):
        ori_image_embeds, image_atts = self.get_vision_embeds(video, video_mask)
        ...
        image_embeds, caption_embeds, ori_caption_embeds = self.interaction_with_caption(image_embeds=ori_image_embeds, 
                                                     caption_embeds=caption_embeds, key_padding_mask=key_padding_mask, video_mask=image_atts)
        image_embeds = image_embeds.transpose(0,1).contiguous() # bsz,n,d
        ########  generated caption  ########

        loss_cv = self.caption_vision_loss(ori_caption_embeds.contiguous(), ori_image_embeds.contiguous(), idx) * self.weight_cv_loss
        loss_reg_c = self.caption_regularization(ori_caption_embeds)
        # loss_reg_c = self.caption_regularization_1(ori_caption_embeds)
        
        image_feat = self.get_features(image_embeds, None, vis_mask=video_mask.unsqueeze(-1))
        ...
        loss_dstl = self.dstl_loss(image_feat.contiguous(), _caption_embeds.contiguous(), text_feat_s.contiguous(), text_feat_t.contiguous(), idx) * self.weight_dstl_loss
